<template>
  <div>
    <Button label="Publish" icon="pi pi-plus-circle" size="small" @click="visible = true" />
    <Dialog
    ...
    >
      ...
      <form class="text-center" @submit.prevent="publish">
...
const publish = async () => {
  ...
  try {
    loading.value = true
    const response = await axios.post('/api/news', { ...form.value });
